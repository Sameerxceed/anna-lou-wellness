...
h3(d, "Status — DONE (Thursday afternoon)")
para(d, "Replaced Strapi's broken date picker with a native one. On iPhone, tapping a date field now opens the iOS wheel picker (years and months scroll reliably). On desktop, it's the browser's own calendar with proper month and year navigation.")

h3(d, "What changed")
bullet(d, "Every date field across the site (events, retreats, workshops, cosmic forecasts, testimonials, vault journeys, workshop replays) now uses the native picker.")
bullet(d, "Added a 'Today' quick-set button — one tap to set the field to today's date.")
bullet(d, "Added a 'Clear' button so you can blank out a date easily.")
bullet(d, "You can still type the date manually as YYYY-MM-DD or DD/MM/YYYY in any browser if you prefer.")

para(d, "No setup needed on your side. Open the Align & Amplify entry and tap the date field — you will see the new picker.")
...
bullet(d, "Native date picker across every date field — replaces the broken Strapi one. Works reliably on iPhone Safari.")
