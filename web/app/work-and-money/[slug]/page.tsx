...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (article) {
    const title = article.seoTitle || `${article.title} â€” Work & Money`;
  ...
  if (category) {
    const title = `${category.name} â€” Work & Money`;
    return {
      title,
...
export default async function ArticlePage({ params }: PageProps) {
  ...
  if (!article) {
    ...
    if (category) {
      ...
      return (
        <EditorialFeed
          kicker={`Work & Money Â· ${category.name}`}
    ...
    <>
      ...
      <article className="article-page">
        <div className="article-inner">
          ...
          <ShopTheStory tags={article.shopTags} accentColour={article.category?.colour || '#FFD07A'} />
