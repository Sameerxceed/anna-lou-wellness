...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const [article, category] = await Promise.all([
    ...
    getArticleCategoryBySlug(slug, 'love-and-relationships'),
  ]);
  if (article) {
      ...
      <>
        ...
        <article className="article-page">
          <div className="article-inner">
            ...
            <img src={getStockImage('love-and-relationships', slug)} alt={title} className="article-hero-img" style={{ objectFit: 'cover' }} />
    ...
    <>
      ...
      <article className="article-page">
        <div className="article-inner">
          ...
          <img
            src={article.heroImage || getStockImage('love-and-relationships', slug)}
            alt={article.title}
          ...
          {!article.isFree && <Paywall substackUrl={article.substackUrl} accentColour={article.category?.colour || '#F280AA'} />}

          <ShopTheStory tags={article.shopTags} accentColour={article.category?.colour || '#F280AA'} />
...
.article-breadcrumb a:hover { color:#F280AA; }
...
.article-substack-link { color:#F280AA; text-decoration:none; border-bottom:1px solid currentColor; padding-bottom:1px; }
.article-substack-link:hover { color:#D85688; }
...
.article-content a { color:#6E3A5A; text-decoration:underline; text-decoration-thickness:1px; text-underline-offset:3px; }
.article-content a:hover { color:#5A2E4A; text-decoration-thickness:2px; }
.article-content h1, .article-content h2, .article-content h3, .article-content h4 { font-family:'Work Sans','Helvetica Neue',sans-serif; font-weight:400; color:#231F20; margin:2rem 0 1rem; line-height:1.3; }
.article-content h2 { font-size:1.6rem; }
.article-content h3 { font-size:1.3rem; }
.article-content h4 { font-size:1.1rem; }
.article-content strong { font-weight:600; color:#231F20; }
.article-content em { font-style:italic; }
.article-content ul, .article-content ol { padding-left:1.5rem; margin-bottom:1.5rem; }
.article-content li { margin-bottom:0.4rem; }
.article-content blockquote { border-left:3px solid #6E3A5A; padding-left:1.2rem; margin:1.5rem 0; font-style:italic; color:#5A5A54; }
